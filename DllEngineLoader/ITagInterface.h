...
struct hs_script_op
{
	...
	UINT64 get_addr(bool val)
	{
		static UINT64 base = IModuleInterface::GetBase(IGameInterface::s_modulePath, !val);
		UINT64 addr = base + IModuleInterface::GetOffset(IGameInterface::s_modulePath, (UINT64)this, val);

		return addr;
	}
	UINT64 get_evaluate_addr(bool val)
	{
		static UINT64 base = IModuleInterface::GetBase(IGameInterface::s_modulePath, !val);
		UINT64 addr = base + IModuleInterface::GetOffset(IGameInterface::s_modulePath, *(UINT64 *)&evaluate, val);

		return addr;
	}
	bool match_evaluates(UINT64 offset)
	{
		return get_evaluate_addr(false) == offset;
	}
	void replace(hs_script_op &new_op)
	{
		hs_script_op &op = *(hs_script_op *)this;
		DWORD old;
		VirtualProtect(&op, sizeof(*this), PAGE_READWRITE, &old);
		memcpy(&op, &new_op, sizeof(*this));
		VirtualProtect(&op, sizeof(*this), old, &old);
	}
	void replace_evaluate(LPVOID new_evaluate)
	{
		DWORD old;
		VirtualProtect(&evaluate, sizeof(&evaluate), PAGE_READWRITE, &old);
		memcpy(&evaluate, &new_evaluate, sizeof(&evaluate));
		VirtualProtect(&evaluate, sizeof(&evaluate), old, &old);
	}
};
#pragma pack(pop)
...
{
	static hs_script_op *(&hs_function_table)[1995] = IModuleInterface::Read<hs_script_op *[1995]>(IGameInterface::s_modulePath, 0x180ABC230);
...
struct halo_script
{
	...
	void generate_csv()
	{
		...
		static UINT64 hs_return_evaluate_addrs[2] = { 0x1802A6140, 0x1802AB750 };
		...
		{
			hs_script_op *hs_function = hs_function_get(i);
			UINT64 script_op_addr = hs_function->get_addr(false);
			UINT64 script_evaluate_addr = hs_function->get_evaluate_addr(false);
			const char *str = halo_script(i).c_str();
			bool is_unknown = (strcmp(str, "") == 0);

			bool is_null = false;
			for (int j = 0; j < _countof(hs_null_evaluate_addrs); j++)
			{
				if (hs_function->match_evaluates(hs_null_evaluate_addrs[j]))
				{
					if (_countof(hs_null_evaluate_addrs) > 1)
					{
						if (is_unknown)
							printf("0x%03X, 0x%09llX, hs_unknown%X_op, 0x%09llX, hs_null%i_evaluate\n", i, script_op_addr, i, script_evaluate_addr, j);
						else
							printf("0x%03X, 0x%09llX, hs_%s_op, 0x%09llX, hs_null%i_evaluate\n", i, script_op_addr, str, script_evaluate_addr, j);
					}
					else
					{
						if (is_unknown)
							printf("0x%03X, 0x%09llX, hs_unknown%X_op, 0x%09llX, hs_null_evaluate\n", i, script_op_addr, i, script_evaluate_addr);
						else
							printf("0x%03X, 0x%09llX, hs_%s_op, 0x%09llX, hs_null_evaluate\n", i, script_op_addr, str, script_evaluate_addr);
					}
					is_null = true;
				}
			}
			...
			for (int j = 0; j < _countof(hs_return_evaluate_addrs); j++)
			{
				if (hs_function->match_evaluates(hs_return_evaluate_addrs[j]))
				{
					if (_countof(hs_return_evaluate_addrs) > 1)
					{
						if (is_unknown)
							printf("0x%03X, 0x%09llX, hs_unknown%X_op, 0x%09llX, hs_null%i_evaluate\n", i, script_op_addr, i, script_evaluate_addr, j);
						else
			...
			bool is_base = false;
			if (hs_function->match_evaluates(IModuleInterface::GetBase(IGameInterface::s_modulePath, false)))
			{
				if (is_unknown)
					printf("0x%03X, 0x%09llX, hs_unknown%X_op, 0x%09llX, (null)\n", i, script_op_addr, i, 0ll);
				else
					printf("0x%03X, 0x%09llX, hs_%s_op, 0x%09llX, (null)\n", i, script_op_addr, str, 0ll);
				is_base = true;
			}
	...
	void generate_idc()
	{
		...
		static UINT64(&hs_script_ops)[_last + 1] = IModuleInterface::Read<UINT64[_last + 1]>(IGameInterface::s_modulePath, 0x180ABC230);
		static UINT64 hs_null_evaluate_addrs[1] = { 0x1802A6130 };
		static UINT64 hs_return_evaluate_addrs[2] = { 0x1802A6140, 0x1802AB750 };
		...
		for (int i = 0; i < _last + 1; i++)
		{
			UINT64 script_op_addr = base + IModuleInterface::GetOffset(IGameInterface::s_modulePath, (UINT64)hs_script_ops[i], false);
			UINT8(&script_op)[0x1C] = IModuleInterface::Read<UINT8[0x1C]>(IGameInterface::s_modulePath, script_op_addr);
			UINT64 script_evaluate_addr = base + IModuleInterface::GetOffset(IGameInterface::s_modulePath, *(UINT64 *)&script_op[0x8], false);
			const char *str = halo_script(i).c_str();
			bool is_unknown = (strcmp(str, "") == 0);

			printf("\t// 0x%03X, %s\n", i, is_unknown ? "unknown" : str);
			printf("\tSetType(0x%08llX, \"hs_script_op\");\n", script_op_addr);

			bool is_null = false;
			for (int j = 0; j < _countof(hs_null_evaluate_addrs); j++)
			{
				if (script_evaluate_addr == hs_null_evaluate_addrs[j])
				{
					if (_countof(hs_null_evaluate_addrs) > 1)
					{
						if (is_unknown)
			...
			for (int j = 0; j < _countof(hs_return_evaluate_addrs); j++)
			{
				if (script_evaluate_addr == hs_return_evaluate_addrs[j])
				{
					if (_countof(hs_return_evaluate_addrs) > 1)
					{
						if (is_unknown)
